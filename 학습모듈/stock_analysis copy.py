...
class StockAnalyzer:
    ...
    def analyze_table(self, table_name):
        ...
        if no_buy:
            ...
            # Buy and Sell annotations
            buy_annotations = []
            sell_annotations = []
            bought = False
            for i in range(1, len(df)):
                if df.iloc[i]['low'] < buy_price and not bought:
                    buy_annotations.append((df.index[i], buy_price))
                    bought = True
                if bought and df.iloc[i]['high'] > price_0786:
                    sell_annotations.append((df.index[i], price_0786))
                    break  # 팔았으면 더 이상 체크할 필요 없음

            # Add buy annotations to the plot
            if buy_annotations:
                buy_dates, buy_prices = zip(*buy_annotations)
                addplots.append(mpf.make_addplot(pd.Series(buy_prices, index=buy_dates), type='scatter', markersize=100, marker='^', color='lime', label='Buy'))
            
            # Add sell annotations to the plot
            if sell_annotations:
                sell_dates, sell_prices = zip(*sell_annotations)
                addplots.append(mpf.make_addplot(pd.Series(sell_prices, index=sell_dates), type='scatter', markersize=100, marker='v', color='red', label='Sell'))
